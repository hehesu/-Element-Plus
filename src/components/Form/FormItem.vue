<template>
  <!-- 表单项组件模板 -->
  <div class="el-form-item" ref="formItemRef" :class="{
...
import type { FormItemContext, FormValidateFailure, FormItemRule } from './types'
...
const isRequired = computed(() => {
  return getItemRules.value?.some((rule: FormItemRule) => (rule as RuleItem).required)
})
...
const getTriggeredRules = (trigger?: string) => {
  ...
  if (itemRules) {
    return itemRules.filter((rule: FormItemRule) => {
      if (!rule.trigger || !trigger) return true
      return rule.trigger && rule.trigger === trigger
    }).map(({ trigger, ...rule }: { trigger?: string;[key: string]: any }): RuleItem => rule)
  } else {
    return []
  }
}
